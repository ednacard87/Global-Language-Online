...
import { getB1EspanolUnitPath, PathItem } from "@/lib/course-data";
import { useUser, useFirestore, useDoc, useMemoFirebase, updateDocumentNonBlocking } from '@/firebase';
import { Loader2, ArrowLeft } from "lucide-react";

/**
 * PANEL DINÁMICO DE UNIDADES B1 - ESPAÑOL
 * --------------------------------------
 */

export default function EspanolB1UnitPage() {
  ...
  useEffect(() => {
    ...
    const initialPath = getB1EspanolUnitPath(unitId);
    ...
    const itemsWithLockState = updatedItems.reduce((acc, item, index) => {
        ...
        if (item.href && item.href !== '#') {
            const classKey = `es-b1-${item.href.split('/').pop()}`;
  ...
  // PERSISTENCIA BLINDADA (Sin bucles infinitos)
  useEffect(() => {
    if (isProfileLoading || !studentDocRef || !unitId || isAdmin) return;
    
    const progressKey = `progress_b1_es_unit_${unitId}`;
    ...
    <div className="flex w-full flex-col espanol-dashboard-bg min-h-screen">
      ...
      <main className="flex flex-1 flex-col items-center gap-8 p-4 md:py-12">
        <div className="text-center">
            ...
            <Link href="/espanol/b1" className="text-sm font-bold text-white/80 hover:underline mt-2 inline-block">
                &larr; Volver al Curso B1
            </Link>
        </div>
        <div className="w-full max-w-4xl">
            <MazeGame 
                ...
                description="Completa cada desafío para dominar los temas de esta unidad."
