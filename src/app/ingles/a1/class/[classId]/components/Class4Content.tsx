...
export default function Class4Content() {
    ...
    const progressStorageKey = 'progress_a1_eng_u1_c4_v100_blindado';
    ...
    const initialLearningPath = useMemo((): Topic[] => [
        { key: 'grammar', name: 'Gramática: Genitivo Sajón', icon: GraduationCap, status: 'active' },
        { key: 'ex-genitivo', name: 'Ejercicio Genitivo', icon: PenSquare, status: 'locked' },
        { key: 'wh-questions', name: 'WH Questions', icon: BookOpen, status: 'locked' },
        { key: 'ex-wh', name: 'Ejercicio WH', icon: PenSquare, status: 'locked' },
    ], []);
    ...
    const handleTopicSelect = (key: string) => {
        ...
        if (['grammar', 'wh-questions'].includes(key)) handleTopicComplete(key);
    };
    ...
    const renderContent = () => {
        ...
        switch (selectedTopic) {
            case 'grammar':
                return <Card className="p-6 text-left"><CardTitle>Genitivo Sajón ('s)</CardTitle><CardContent className="pt-4 space-y-4"><p>Indica posesión: Poseedor + 's + Posesión.</p><p className="bg-muted p-2">Example: Mary's car.</p></CardContent><CardFooter><Button onClick={() => handleTopicComplete('grammar')}>Siguiente</Button></CardFooter></Card>;
            case 'ex-genitivo': return <GenitiveCaseExercise onComplete={() => handleTopicComplete('ex-genitivo')} />;
            case 'wh-questions': return <Card className="p-6 text-left"><CardTitle>WH Questions</CardTitle><CardContent className="pt-4 space-y-2"><p>Who, What, Where, When, Why, How.</p></CardContent><CardFooter><Button onClick={() => handleTopicComplete('wh-questions')}>Entendido</Button></CardFooter></Card>;
            case 'ex-wh': return <WhQuestionsMainExercise onComplete={() => handleTopicComplete('ex-wh')} />;
            default: return null;
        }
    };
