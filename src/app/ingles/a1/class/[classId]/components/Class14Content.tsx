...
import { BookOpen, PenSquare, Lock, CheckCircle, Loader2, Trophy } from 'lucide-react';
...
const progressStorageVersion = 'progress_a1_eng_u3_c14_v30_blindado';
...
export default function Class14Content() {
    ...
    const initialLearningPath = useMemo((): Topic[] => [
        ...
        { key: 'last_ex', name: 'Final', icon: CheckCircle, status: 'locked' },
    ], []);
    ...
    const renderContent = () => {
        ...
        switch (selectedTopic) {
            case 'vocabulary':
                return (
                    <Card className="shadow-soft border-2 border-brand-purple">
                        ...
                        <CardContent><div className="space-y-4 text-left"><div><h4 className="font-bold">Verbos</h4><div className="grid grid-cols-2 gap-2">{vocabularyData.verbs.map((v, i) => (<React.Fragment key={i}><div className="p-2 border rounded bg-muted/10">{v.spanish}</div><Input value={vocabAnswers.verbs[i]} onChange={e => { const n = {...vocabAnswers}; n.verbs[i] = e.target.value; setVocabAnswers(n); setCanAdvanceVocab(true); }} /></React.Fragment>))}</div></div></div></CardContent>
            ...
            case 'last_ex': return <Card className="p-12 text-center shadow-soft border-2 border-brand-purple"><Trophy className="h-20 w-20 mx-auto text-yellow-400" /><h2 className="text-2xl font-bold mt-4">¡Clase 14 Completada!</h2><Button onClick={() => setTopicToComplete('last_ex')} className="mt-8">Finalizar</Button></Card>;
            default: return null;
        }
    };
