...
const progressStorageVersion = 'progress_a1_eng_u3_c13_v30_blindado';
...
export default function Class13Content() {
    ...
    const handleTopicSelect = (topicKey: string) => {
        ...
        if (!isAdmin && it?.status === 'locked') return;
            ...
            case 'vocabulario':
                return (
                    <Card className="shadow-soft border-2 border-brand-purple">
                        ...
                        <CardFooter className='flex justify-between'><Button onClick={() => { let ok = false; vocabularyData.forEach((v, i) => { if (v.english.some(e => e.toLowerCase() === vocabAnswers[i].trim().toLowerCase())) ok = true; }); setCanAdvanceVocab(ok); if (ok) toast({ title: "¡Bien!" }); }}>Verificar</Button><Button onClick={() => setTopicToComplete('vocabulario')} disabled={!canAdvanceVocab && !isAdmin}>Avanzar</Button></CardFooter>
                    </Card>
                );
            case 'comparativos': return <Card className="p-6 text-left"><CardTitle>COMPARATIVOS</CardTitle><CardContent className="pt-4 space-y-4"><p>Adjetivo + ER + THAN (Más que).</p></CardContent><CardFooter><Button onClick={() => setTopicToComplete('comparativos')}>Siguiente</Button></CardFooter></Card>;
            case 'ejercicio-comparativo': return <ComparativeExercise onComplete={() => setTopicToComplete('ejercicio-comparativo')} />;
            case 'superlativos': return <Card className="p-6 text-left"><CardTitle>SUPERLATIVOS</CardTitle><CardContent className="pt-4 space-y-4"><p>THE + Adjetivo + EST (El más).</p></CardContent><CardFooter><Button onClick={() => setTopicToComplete('superlativos')}>Siguiente</Button></CardFooter></Card>;
