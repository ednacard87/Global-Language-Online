...
import { getKidsA2MainPath, PathItem } from "@/lib/course-data";
...
export default function KidsA2CoursePage() {
  ...
  useEffect(() => {
    ...
    const updatePath = () => {
        const initialPath = getKidsA2MainPath(t);
        ...
        const itemsWithLockState = itemsWithProgress.map((item, index, arr) => {
            return { ...item, locked: false }; // Unlocking all for now
        });
  ...
  return (
    <div className="a2-kids-bg flex w-full flex-col min-h-screen">
      <DashboardHeader />
                ...
                >
                    <CardContent className="p-8 pt-4 border-t">
                        <CardHeader className="p-0">
                            ...
                            <CardDescription>{isClient ? t('kidsA1.overallProgressDescription') : 'Your total progress in the A2 kids course.'}</CardDescription>
                        </CardHeader>
                        <div className="pt-4">
