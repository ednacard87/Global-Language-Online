...
import { getB2MainPath, PathItem } from "@/lib/course-data";
import { useTranslation } from "@/context/language-context";
import { useUser, useFirestore, useDoc, useMemoFirebase } from '@/firebase';
import { doc } from 'firebase/firestore';

export default function B2CoursePage() {
  ...
  useEffect(() => {
    ...
    const updatePath = () => {
        const initialPath = getB2MainPath(t);

        const progressU1 = studentProfile?.progress?.['progress_b2_unit_1'] || 0;
        const progressU2 = studentProfile?.progress?.['progress_b2_unit_2'] || 0;
    ...
    <div className="flex w-full flex-col ingles-dashboard-bg min-h-screen">
      ...
      <main className="flex flex-1 flex-col items-center gap-8 p-4 md:py-12">
        <div className="text-center">
            <h1 className="text-4xl font-bold text-brand-purple dark:text-primary [text-shadow:1px_1px_1.5px_hsl(var(--accent)/0.8)]">{t('dashboard.courseB2')}</h1>
        </div>
