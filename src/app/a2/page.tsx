...
import { getA2EspanolPath, PathItem } from "@/lib/course-data";
...
export default function A2CoursePage() {
  ...
  useEffect(() => {
    ...
    const updatePath = () => {
        const initialPath = getA2EspanolPath(t);
    ...
    <div className="flex w-full flex-col espanol-dashboard-bg min-h-screen">
      ...
      <main className="flex flex-1 flex-col items-center gap-8 p-4 md:py-12">
        <div className="text-center">
            <h1 className="text-4xl font-bold text-brand-purple dark:text-primary [text-shadow:1px_1px_1.5px_hsl(var(--accent)/0.8)]">{t('a2Espanol.title')}</h1>
        </div>
