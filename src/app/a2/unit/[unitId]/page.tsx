...
import { getA2UnitPath, PathItem } from "@/lib/course-data";
...
export default function A2UnitPage() {
  ...
  useEffect(() => {
    ...
    const updatePath = () => {
        const initialPath = getA2UnitPath(unitId, t);
  ...
  useEffect(() => {
    ...
    if (studentDocRef && unitId) {
        updateDocumentNonBlocking(studentDocRef, {
            [`progress.progress_a2_unit_${unitId}`]: unitProgress
        });
    ...
    <div className="flex w-full flex-col">
      ...
      <main className="flex flex-1 flex-col items-center gap-8 p-4 md:py-12">
        <div className="text-center">
            <h1 className="text-4xl font-bold text-brand-purple dark:text-primary [text-shadow:1px_1px_1.5px_hsl(var(--accent)/0.8)]">{isClient ? t('a2course.unitTitle', { unit: unitId }) : ''}</h1>
            <Link href="/a2" className="text-sm text-muted-foreground hover:underline mt-2 inline-block">
                &larr; {isClient ? t('a2course.backToA2') : ''}
            </Link>
        </div>
        <div className="w-full max-w-7xl">
            <MazeGame 
                pathItems={pathItems} 
                title={isClient ? t('a2course.unitPath') : ''}
                description={isClient ? t('a2course.unitPathDescription') : ''}
