...
import { getA1UnitPath, PathItem } from "@/lib/course-data";
...
export default function A1UnitPage() {
  ...
  useEffect(() => {
    ...
    const updatePath = () => {
        const initialPath = getA1UnitPath(unitId, t);
  ...
  useEffect(() => {
    ...
    if (studentDocRef && unitId) {
        updateDocumentNonBlocking(studentDocRef, {
            [`progress.progress_a1_unit_${unitId}`]: unitProgress
        });
    ...
    <div className="flex w-full flex-col">
      ...
      <main className="flex flex-1 flex-col items-center gap-8 p-4 md:py-12">
        <div className="text-center">
            <h1 className="text-4xl font-bold text-brand-purple dark:text-primary [text-shadow:1px_1px_1.5px_hsl(var(--accent)/0.8)]">{isClient ? t('a1course.unitTitle', { unit: unitId }) : ''}</h1>
            <Link href="/a1" className="text-sm text-muted-foreground hover:underline mt-2 inline-block">
                &larr; {isClient ? t('a1course.backToA1') : ''}
            </Link>
        </div>
