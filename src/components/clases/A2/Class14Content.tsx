...
const progressStorageVersion = 'progress_a2_eng_u2_c14_v1_base';
const mainProgressKey = 'progress_a2_eng_unit_2_class_14';
...
export default function Class14Content({ overrideStudentId }: { overrideStudentId?: string | null }) {
    ...
    // Definición de la ruta de aprendizaje de 18 pasos
    const initialPathData = useMemo((): Topic[] => [
        { key: 'vocabulary_health', name: '1. Vocabulary (Health Issues)', icon: HeartPulse, status: 'active' },
        { key: 'grammar_since', name: '2. Grammar (Since)', icon: GraduationCap, status: 'locked' },
        { key: 'ex_since', name: '3. Exercise with Since', icon: PenSquare, status: 'locked' },
        { key: 'grammar_for_from', name: '4. Grammar (For & From)', icon: Clock, status: 'locked' },
        { key: 'ex_for', name: '5. Exercise with For', icon: PenSquare, status: 'locked' },
        { key: 'grammar_ago', name: '6. Grammar (Ago)', icon: History, status: 'locked' },
        { key: 'ex_ago', name: '7. Exercise with Ago', icon: PenSquare, status: 'locked' },
        { key: 'ex_mix_1', name: '8. Exercise Mix 1', icon: Activity, status: 'locked' },
        { key: 'vocab_game', name: '9. Vocabulary (Game)', icon: Gamepad2, status: 'locked' },
        { key: 'ex_mix_2', name: '10. Exercise Mix 2', icon: Activity, status: 'locked' },
        { key: 'grammar_tag_q', name: '11. Grammar (Tag Questions)', icon: HelpCircle, status: 'locked' },
        { key: 'ex_tag_q', name: '12. Exercise with Tag Q.', icon: PenSquare, status: 'locked' },
        { key: 'create_tag_q_1', name: '13. Create Tag Q.', icon: Pencil, status: 'locked' },
        { key: 'reading', name: '14. Reading', icon: BookText, status: 'locked' },
        { key: 'ex_mix_3', name: '15. Exercise Mix 3', icon: Activity, status: 'locked' },
        { key: 'fill_gaps_1', name: '16. Fill the Gaps 1', icon: FileText, status: 'locked' },
        { key: 'create_tag_q_2', name: '17. Create Tag Q. 2', icon: Pencil, status: 'locked' },
        { key: 'final_exercise', name: '18. Final Exercise', icon: Trophy, status: 'locked' },
    ], []);

    // ASYNC FLOW 1: Carga inicial de Firestore y reconstrucción de estados (ÚNICA VEZ)
    useEffect(() => {
        ...
        // Reparación de ruta lógica secuencial para alumnos
        if (!isAdmin || targetStudentId) {
            let lastDone = true;
    ...
    // ASYNC FLOW 3: Manejo de desbloqueos asíncronos (Toaster safe)
    useEffect(() => {
            ...
            <Card className="shadow-soft border-2 border-brand-purple bg-card/95 backdrop-blur-sm text-foreground text-left overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
                <CardHeader className='bg-primary/5 border-b'>
                    <div className='flex items-center gap-3'>
                        ...
                        <div>
                            ...
                            <CardDescription className='font-bold text-foreground'>Sección de aprendizaje para la Clase 14 (A2).</CardDescription>
                        </div>
                    </div>
                </CardHeader>
                <CardContent className="py-20 flex flex-col items-center justify-center min-h-[400px] text-center space-y-6">
                    ...
                    <div className='space-y-2'>
                        ...
                        <p className='text-muted-foreground text-xl'>El contenido interactivo para "{topic.name}" se está cargando en el búnker de misiones.</p>
                    </div>
                </CardContent>
                ...
                <Card className="shadow-soft rounded-lg sticky top-24 border-2 border-brand-purple bg-card/95 backdrop-blur-sm">
                    <CardHeader className="pb-4 border-b bg-muted/30">
                        <CardTitle className="text-lg font-black text-primary uppercase flex items-center gap-2">
                            <Trophy className="h-5 w-5 text-primary" /> Misión 14A
                        </CardTitle>
                    </CardHeader>
