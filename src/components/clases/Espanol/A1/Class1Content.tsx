...
/**
 * COMPONENTE DE CLASE 1 (ESPAÑOL A1) - MÓDULO INDEPENDIENTE
 * -------------------------------------------------------
 * Este componente es el primer pilar de la suite de Español.
 * Implementa el "Blindaje Light" para asegurar la estabilidad.
 */
...
export default function Class1Content() {
    ...
    useEffect(() => {
        if (!isUserLoading && !isProfileLoading) {
            // Sincronización breve para asegurar que el estado de Firebase esté listo
    ...
    if (isInitialLoading) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[400px]">
                ...
                <p className="text-white font-bold tracking-widest animate-pulse uppercase">Sincronizando Misión Español...</p>
            </div>
        );
    }
                    ...
                    <CardContent className="text-center space-y-8 pb-10">
                        ...
                        <div className="flex justify-center">
                            <Button asChild variant="default" size="lg" className="rounded-full px-10 font-bold h-14 shadow-lg bg-primary hover:bg-primary/90 text-primary-foreground transition-all active:scale-95">
                                <Link href="/espanol">
                                    <ArrowLeft className="mr-2 h-5 w-5" /> Volver al panel de Español
                                </Link>
                            </Button>
                        </div>
                    </CardContent>
                    <CardFooter className="justify-center border-t pt-4 bg-muted/20">
                        <p className="text-xs text-muted-foreground uppercase font-bold tracking-widest">Global English Online - Suite Español Blindada</p>
                    </CardFooter>
                </Card>
            </div>
