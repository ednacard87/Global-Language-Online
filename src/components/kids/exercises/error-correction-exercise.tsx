...
export type ErrorCorrectionPrompt = {
    incorrect: string;
...
interface ErrorCorrectionExerciseProps {
    exerciseData: ErrorCorrectionPrompt[];
    onComplete: () => void;
    title: string;
}

export function ErrorCorrectionExercise({ exerciseData, onComplete, title }: ErrorCorrectionExerciseProps) {
    ...
    const handleFinalCheck = () => {
        let allCorrect = true;
        const newValidationStates = exerciseData.map((prompt, index) => {
            const userAnswer = userAnswers[index]?.trim().toLowerCase().replace(/[.?,]/g, '') || '';
            const correctAnswers = prompt.correctAnswers.map(a => a.toLowerCase().replace(/[.?]/g, ''));
    ...
    return (
        <Card className="shadow-soft rounded-lg border-2 border-brand-purple">
            <CardHeader>
                ...
                <CardDescription>Encuentra el error y corrige la frase.</CardDescription>
            ...
            <CardContent className="space-y-6">
                <div>
                    <h3 className="text-lg font-semibold mb-2">Frase con error:</h3>
                    <div className="bg-destructive/10 p-4 rounded-lg border border-destructive">
                        <p className="text-lg font-medium text-destructive">{currentPrompt?.incorrect}</p>
                        <p className="text-sm text-muted-foreground italic mt-1">{currentPrompt?.translationHint}</p>
                    </div>
                </div>
                <div>
                    <Input
            ...
            <CardFooter className="flex justify-between items-center">
                ...
                {currentIndex === exerciseData.length - 1 ? (
                     ...
                     <Button onClick={() => setCurrentIndex(p => Math.min(exerciseData.length - 1, p + 1))}>
                         Siguiente
