...
import { alphabetWithPronunciation } from '@/components/kids/alphabet-grid';

export const AbcMemoryGame = ({ onGameComplete }: { onGameComplete: () => void }) => {
    ...
    const initializeGame = (lvl: number) => {
        ...
        const alphabetPairs = alphabetWithPronunciation
            .slice(0, pairs)
            .map(item => ({ letter: item.letter, pronunciation: `(${item.pronunciation})` }));
        
        const gameCards = alphabetPairs.flatMap((pair, index) => [
            { id: index * 2, pairId: index, text: pair.letter },
            { id: index * 2 + 1, pairId: index, text: pair.pronunciation },
        ]);
    ...
    return (
        <Card className="shadow-soft rounded-lg border-2 border-brand-purple">
            <CardHeader>
                <CardTitle>Memory (ABC)</CardTitle>
