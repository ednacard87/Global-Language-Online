...
import { verbVocabularyData as defaultVerbVocabularyData } from './verb-vocabulary';


export const VerbMemoryGame = ({ onComplete, data }: { onComplete: () => void, data?: {spanish: string, english: string}[] }) => {
    ...
    const verbData = data || defaultVerbVocabularyData;

    const levelConfig = useMemo(() => {
        const shuffledVerbs = [...verbData].sort(() => 0.5 - Math.random());
        return {
            1: { pairs: shuffledVerbs.slice(0, 6), grid: 'grid-cols-4', name: 'Nivel 1' },
            2: { pairs: shuffledVerbs.slice(0, 8), grid: 'grid-cols-4', name: 'Nivel 2' },
            3: { pairs: shuffledVerbs.slice(0, 10), grid: 'grid-cols-5', name: 'Nivel 3' },
        };
    }, [verbData]);
    ...
    }, [isLevelComplete, level, onComplete, levelConfig, completionFired]);
    ...
    return (
        <Card className="shadow-soft rounded-lg border-2 border-brand-purple">
            <CardHeader>
                <CardTitle>Memory (Verbos)</CardTitle>
